...
We extract the @-message graph from a large crawl of Twitter that took place between August 2009 and January 2010.
More than three-billion messages from over 60 million users were collected.  
The graph $G = (V,E)$ of users who authored at least one @-message contains 12,795,683 unique users ($|V|$) who sent a total of 819,305,776 @-messages, with 156,868,257 unique directed interactions ($|E|$). 

We focus our analysis on the subgraph $G_{1000}$, the subgraph induced by users who authored at least 1000 @-messages. 
Furthermore, we only consider an edge $(v,w)$ to be present if at least 10 @-messages were sent from $v$ to $w$.
In $G_{1000}$, we find $|E^r_{10}| = 797,342$, $|E^u_{10}| = 349,258$.
...
We then combined subsets of features and evaluated their performance, by splitting the edges in $E^r_{10} \cup E_{10}^u$ randomly into two sets, training on one set and evaluating on the other.
