...
As social media applications becomes richer and more complex,
...
(for our purposes, @-messages directed from one user to another)
...
The challenge, when studying an environment such as Twitter,
is that all of these types of connections are superimposed 
in a single communication network.
As a result, it is important to develop techniques capable of dissecting
the links of the underlying network into these fundamentally
different forms of activity.
Here we develop a set of techniques for this purpose,
...
notion of status --- roughly speaking,
people with similar status often participate in reciprocal
...
prediction are not the ones that are most effective for 
reciprocity prediction.

There has also been recent research predicting the 
{\em strengths} \cite{gilbert-tie-strength} and the 
{\em signs} \cite{leskovec-chi10} of links in an on-line social network.
These too form interesting contrasts with reciprocity prediction.
In particular, it is possible for a strong directed tie to be
either reciprocated or unreciprocated.  (For example, an avid
follower of the New York Times might regular generate tweets
that say things such as, ``@nytimes reports that ... '' without
ever receiving a message from the @nytimes Twitter account.)
There is also a connection to the problem of sign prediction in that
notions of status are crucial there too \cite{leskovec-chi10},
...
that $v$ follows $w$) --- this is in keeping with our focus 
on communication activities.
Our task at a general level
...
In the second formulation, we ask whether $(w,v)$ exists given that the 
$(v,w)$ edge exists.
...
predicting lack of reciprocity in the complementary region.
...
For each feature, we determine the threshold value $\operatorname{val}_{OPT}$ and threshold direction (less than / greater than), so as to maximize prediction accuracy according to this threshold classifier.
...
sends, regardless of the nodes to which messages are sent or from
which  messages are received.  Specifically, we use the analogous measures
...
If $v$ and $w$ receive many messages from the same group of people, it could be that both $v$ and $w$ are in the same community, or that both $v$ and $w$ are celebrities (who may or may not talk to each other) and there is an overlapping fan-base.
...
Indeed, if we only consider edges $e \in {(v,w)~|~\deg^-(v) = \deg^-(w)}$, or that both nodes in every node pair have equal indegree, the accuracy of the outdegree ratio feature rises to 0.811, compared to 0.628 without this filtering. This result is comparable to what we get for the outdegree-indegree ratio feature.
...
If we only consider the set of edges node pairs with both nodes in a pair having equal indegree ($|E|=16,311$), the accuracy of All ratio rises to 0.927, suggesting that predicting reciprocity among nodes which have equal status is a relatively easier task (?). Here, the most important feature is outdegree ratio. but this is unsurprising, since outdegree-indegree ratio reduces to outdegree ratio in the case of equal indegrees. 
